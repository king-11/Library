...
books.on("child_added", (snap) => {
  ...
  let reader = snap.val().read.trim();
  hallOfFame.has(reader) ?
    hallOfFame.set(reader, hallOfFame.get(reader) + 1) :
    hallOfFame.set(reader, 1);
});
...
function setContent(book, id) {
  ...
  $.get(
    ...
    function (response) {
      if (!response.items) {
        cardText.innerHTML = `<p style="font-size:1rem; font-family:'Pacifico">The Genre of this book is ${
          book.pages
        } and it was recommended by : ${book.read.bold()}.</p>`;
        image.setAttribute(
          "src",
          `./assets/css/{book.pages.toLowerCase()}.jpg`
        );
        cardText.appendChild(image);
      }

      if (response.items[0].volumeInfo.description) {
        let descText = response.items[0].volumeInfo.description;
        cardText.innerHTML += `<p id="large" style="font-size:1rem; font-family:'Handlee">${descText.slice(
          0,
          400
        )} ...</p>`;

        cardText.innerHTML += `<p id="medium" style="font-size:1rem; font-family:'Handlee">${descText.slice(
          0,
          300
        )} ...</p>`;

        cardText.innerHTML += `<p id="small" style="font-size:0.8rem; font-family:'Handlee">${descText.slice(
          0,
          150
        )} ...</p>`;
      } else if (response.items[0].searchInfo) {
        let descText = response.items[0].searchInfo.textSnippet;
...
let Panel = () => {
  let main = document.querySelector("#people");
  main.innerHTML = "";

  hallOfFame = new Map([...hallOfFame].sort((a, b) => a[1] < b[1]))

  hallOfFame.forEach((value, key) => {
    let person = document.createElement("li");
    src = `"https://ui-avatars.com/api/?name=${key}&rounded=true"`;
    person.innerHTML = `<img src=${src}></img>
                        <p>${key}</p>`;
    main.append(person);
  });
};
...
function openContri() {
  let a = document.querySelector("#contri");
  a.style.width = "100%";
  Panel();
}
